...
public:
    ...
    Y get_y(const X &x) const {
        if (x < first || number == 0)
            throw std::out_of_range("x is outside the range of the segment");

        if (one_point()) {
            return static_cast<Y>((rectangle[0].y + rectangle[1].y) / 2);
        }

        auto[min_slope, max_slope] = get_slope_range();
        auto slope = (min_slope + max_slope) / 2.;
        auto [i_x, i_y] = get_intersection();
        auto intercept = i_y - (i_x - first) * slope;
        return static_cast<Y>(slope * (x - first) + intercept);
    }
};
